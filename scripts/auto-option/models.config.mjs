...
/*
 * ── 알려진 한계: 등급 계열이 한 트림에서 둘 다 "기본" 으로 잡힌다 ──────────
 *
 * 예: 아반떼 Inspiration 에서 현대 스마트센스 Ⅰ 과 Ⅱ 가 둘 다 기본.
 *
 * 이게 항상 틀린 건 아니다. 두 종류가 섞여 있다.
 *   누적형 — 스마트센스는 Ⅰ 이 기본이 된 뒤 Ⅱ 를 40만에 더한다. 둘 다 기본이 맞다.
 *            기아 원본 격자도 스포티지 시그니처에 컴포트 Ⅰ+Ⅱ 를 둘 다 기본으로 적는다.
 *   배타형 — 플래티넘은 Ⅰ 93만 / Ⅱ 90만으로 Ⅱ 가 더 싸다. 대체 관계다.
 *
 * 이름으로는 구분이 안 되고, 원본의 배타 표기("(택1)", "중복 선택 불가")는
 * 28종 통틀어 5건뿐이다(투싼은 0건인데 배타 계열이 있다).
 *
 * **고치지 않기로 했다.** 기본은 추가금 0원이라 총액과 "최소 트림" 답이 안 바뀐다.
 * 틀리는 건 옵션 격자의 표시뿐이다. 반면 배타형으로 잘못 판단해 하위 등급을
 * '불가' 로 돌리면, 트림을 탈락시켜 더 비싼 차를 추천하는 심각한 오류가 된다 —
 * 기아 원본으로 실측했을 때 그 추론은 94% 틀렸다.
 *
 * 근거가 생기면(택1 표기가 늘거나 다른 단서) 그때 다룬다.
 */
