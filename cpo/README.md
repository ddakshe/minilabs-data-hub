...
메모리 벽을 우회하려고 `sorting=PRODUCTION_DATE_ASC` 로 한 번 더 돌아 오래된 쪽을
긁어봤다. 건수 계산은 맞았다(최신 700 + 오래된 700 = 1,400 > 전체 1,339).

**쓸모가 없었다.** 추가된 36건 중 35건(97%)에 `modelYear` 가 없다:

```
year=None  X1 xDrive20i   2,390만원
year=None  118d 5-doors   1,030만원
year=None  420d           2,390만원
→ 실제로 쓸 수 있는 추가분: 1건
```

생산일이 가장 오래된 매물이라 BMW 가 연식을 채우지 않는다. 앱은 연식이 기본 정렬축이자
필터축이어서 이 레코드를 쓸 수 없다.

**정렬 반전은 "데이터가 얇은 쪽"을 긁는 방식이라 반대쪽 끝일수록 품질이 낮다.**
`BMW_SORTING` 환경변수는 남겨뒀지만 기본값(DESC)을 바꿀 이유는 없다.
