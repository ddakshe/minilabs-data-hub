...
- 🚨 2026-09-01 부터 1,500 → 300 으로 **하향**됐다. 앱 런타임 호출 금지 — 사용자 100명이 하루 3번 열면 그날로 소진이다. 무료 한도를 5분의 1로 조인 전례가 있으니 받은 과거 데이터는 저장소에 영구 누적한다.

| 워크플로 | 주기(회/일) | 콜/회 | 평균/일 | 비고 |
|---|---:|---:|---:|---|
| `fetch-oil.yml` | 1 | 2 | 2 | avgRecentPrice(최근 7일) 1 + avgAllPrice(오늘 잠정) 1. 매일 07:40 KST |
| `probe-oil.yml` | 예정 | 130 | — | 📋 **예정** · 백테스트 5년치 수집. 7일 단위라 총 261회지만 한도가 300 뿐이라 **이틀에 나눠(130/일) 돈다** — 한 번에 돌리면 그날 fetch-oil 까지 위태롭다. 수동 실행. 「특정 7일간」 엔드포인트 이름 미확인이라 아직 파일이 없다 |
| **합계** | | | **2** | 🟢 평균은 한도의 **1%** |

> **최대 1회 소모 130건 (한도의 43%).** 한도를 깨는 것은
> 평균이 아니라 **버스트**다 — 무거운 배치가 도는 날은 같은 키를 쓰는 다른
> 워크플로와 부딪힌다. 큰 배치는 날짜를 겹치지 않게 두거나 나눠 돌린다.
