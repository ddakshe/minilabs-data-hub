...
| `fetch-oil-weekly.yml` | 0.1 | 20 | 2.9 | 원/달러 일별 2010~ 을 1000행씩 페이지로 받는다(약 17페이지). 주 1회 |
| **합계** | | | **4.6** | 🟢 평균은 한도의 **0%** |

> **최대 1회 소모 20건 (한도의 0%).** 한도를 깨는 것은
> 평균이 아니라 **버스트**다 — 무거운 배치가 도는 날은 같은 키를 쓰는 다른
> 워크플로와 부딪힌다. 큰 배치는 날짜를 겹치지 않게 두거나 나눠 돌린다.
...
- **한국석유공사 오피넷 유가정보 API (일반)** · 한도 **300건/일**
- 범위: 키 단위 — 이 키를 쓰는 모든 워크플로가 **나눠 쓴다**. 현재 oil 전용이라 300 전부가 우리 예산이다.
- 🚨 2026-09-01 부터 1,500 → 300 으로 **하향**됐다. 앱 런타임 호출 금지 — 사용자 100명이 하루 3번 열면 그날로 소진이다. 무료 한도를 5분의 1로 조인 전례가 있으니 받은 과거 데이터는 저장소에 영구 누적한다. 과거 소급은 API가 아니라 웹 폼(scripts/fetch-oil-history.mjs)으로 받으므로 이 한도를 쓰지 않는다.

| 워크플로 | 주기(회/일) | 콜/회 | 평균/일 | 비고 |
|---|---:|---:|---:|---|
| `fetch-oil.yml` | 1 | 6 | 6 | avgRecentPrice 1 + avgAllPrice 1 + avgSidoPrice 4유종. 매일 07:40 KST |
| **합계** | | | **6** | 🟢 평균은 한도의 **2%** |

> **최대 1회 소모 6건 (한도의 2%).** 한도를 깨는 것은
> 평균이 아니라 **버스트**다 — 무거운 배치가 도는 날은 같은 키를 쓰는 다른
> 워크플로와 부딪힌다. 큰 배치는 날짜를 겹치지 않게 두거나 나눠 돌린다.
...
스캔: 워크플로 46개 · 관리 대상 키 20개 · 2026-09-05
