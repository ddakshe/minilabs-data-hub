...
1. **롯데 스파인** (가장 깨끗): 롯데 현재상영 → 스파인, KOFMovieCd로 KOBIS 조인, KMDb 줄거리, 신규 스키마. 산출·검증.
2. **메가박스 추가**: playwright AJAX 인터셉트, 제목+개봉연도 dedup 병합, `imgPathNm` 포스터 URL 확정.
3. **CGV 추가**: 현재상영 셀렉터 확정(playwright), 병합.
4. **워크플로우 playwright 대응 + end-to-end 검증**.

각 단계는 독립적으로 동작·검증 가능(1단계만으로도 기존보다 개선).
