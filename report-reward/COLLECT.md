...
샘플 4곳을 먼저 읽어라. 형태를 하나씩 보여준다.

| 파일 | 형태 |
|---|---|
| `regions/41110.json` 수원시 | 과태료의 20% (method `rate`) · 포상금이 **별도 과태료 조례**에 있다 |
| `regions/11680.json` 강남구 | 유형별 지급률 (method `rate` — 일반 50%·담배꽁초 20%) · **별표 5 가 금액표가 아니라 비율 규정**이었다. 별표라고 금액표로 단정하지 않는다 |
| `regions/51720.json` 홍천군 | 금액표 (method `table`, examples 10행, hwp5html 로 읽음) · 조례(기한·증거) + **시행규칙**(금액·자격)에 나뉘어 있다 |
| `regions/41280.json` 고양시 | 금액표를 **못 읽은** 곳 (`tableOnly: true`) — pyhwp 가 별표 10 을 못 연다. 앱은 원문 링크만 보여준다 |
...
- `rates[].item` 이 「과태료 부과금액」처럼 **무엇에 대한 비율인지 안 보이면** 앱 타일에 뜻 없는 라벨이 뜬다 — 「그 밖의 무단투기」처럼 대상을 적는다(강남 50% 행).
- item 을 줄일 때는 **같은 파일 안에서 두 행이 같아지지 않게** 한다(양주 소각 2행). 검증기가 중복·「…행위」 끝을 경고한다.
